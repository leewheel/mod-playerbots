...
bool BisGearAction::RunAutogearFallback(uint16 effectiveIlvl)
{
    ...
    botAI->TellMaster(PlayerbotTextMgr::instance().GetBotTextOrDefault(
        ...
        "没有适合你等级/专精/层级的 BiS，请检查配置，改为运行自动装备", {}));
...
bool BisGearAction::Execute(Event event)
{
    ...
    {
        botAI->TellError(PlayerbotTextMgr::instance().GetBotTextOrDefault(
            "bis_altbot_refused_error", "无法在 alt 机器人上使用 bis。", {}));
        return false;
    }
    ...
    {
        botAI->TellError(PlayerbotTextMgr::instance().GetBotTextOrDefault(
            "bis_quality_floor_error", "AutoGearQualityLimit 必须为 4 才能使用 BiS。", {}));
        return false;
    }
    ...
    {
        botAI->TellError(PlayerbotTextMgr::instance().GetBotTextOrDefault(
            "bis_pvp_refused_error", "bis 仅限 PvE，机器人配置为 PvP。", {}));
        return false;
    }
    ...
    if (bisMap.empty())
    {
        ...
        botAI->TellMaster(PlayerbotTextMgr::instance().GetBotTextOrDefault(
            ...
            "ilvl %ilvl 没有 BiS，改用自动装备 %ilvl", phs));
    ...
    if (resolvedIlvl != ilvl)
    {
        ...
        botAI->TellMaster(PlayerbotTextMgr::instance().GetBotTextOrDefault(
            ...
            "ilvl %requested 没有 BiS，使用最接近的 ilvl %resolved", phs));
    }

    botAI->TellMaster(PlayerbotTextMgr::instance().GetBotTextOrDefault(
        "bis_applying_msg", "正在应用 BiS 装备", {}));
        ...
        botAI->TellMaster(
            ilvl != 0
                ? PlayerbotTextMgr::instance().GetBotTextOrDefault(
                      ...
                      "正在清空装备并从头重新装备(品质 %quality，目标物品等级 %ilvl)。", announcePhs)
                : PlayerbotTextMgr::instance().GetBotTextOrDefault(
                      "autogear_reset_start_any_ilvl_msg", "正在清空装备并从头重新装备(品质 %quality)。",
                      announcePhs));
    else
        botAI->TellMaster(
            ilvl != 0 ? PlayerbotTextMgr::instance().GetBotTextOrDefault(
                            "autogear_start_msg", "正在自动装备(品质 %quality，目标物品等级 %ilvl)。", announcePhs)
                      : PlayerbotTextMgr::instance().GetBotTextOrDefault("autogear_start_any_ilvl_msg",
                                                                         "正在自动装备(品质 %quality)。", announcePhs));
