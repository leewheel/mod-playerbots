...
const Position KALECGOS_TANK_POSITION = {
    1703.584f, 895.626f, 53.076f };
const Position KALECGOS_INITIAL_RANGED_POSITION = {
    1704.634f, 938.080f, 53.076f };
...
uint8 GetKalecgosAssignedGroup(
    const KalecgosEncounterState& state, ObjectGuid playerGuid)
{
...
KalecgosEncounterState& GetPreparedKalecgosEncounterState(
    ...
    Player* candidate, const KalecgosEncounterState& state)
{
    ...
    return GetKalecgosAssignedGroup(
        state, candidate->GetGUID()) == state.activeRiftGroup;
}
...
void AnnounceKalecgosTankTransition(PlayerbotAI* botAI, std::string const& textId,
    ...
    const std::array<ObjectGuid, KALECGOS_TANK_COUNT>& tankAssignmentGuids)
{
    ...
    auto const appendGuid = [&](ObjectGuid guid)
    {
        ...
        if (std::find(rotationGuids.begin(), rotationGuids.end(), guid) ==
                rotationGuids.end())
        {
...
{
    return candidate &&
           HasKalecgosTankAssignment(state.tankAssignmentGuids, candidate->GetGUID());
}
...
uint8 CountKalecgosSurfaceAssignedTanks(
    Group* group, const KalecgosEncounterState& state)
{
    uint8 count = 0;
    for (ObjectGuid guid : state.tankAssignmentGuids)
...
Player* GetKalecgosCurrentVictimTank(
    PlayerbotAI* botAI, Player* bot, Group* group,
    const KalecgosEncounterState& state)
{
    ...
    Group* group, const KalecgosEncounterState& state, Player* currentTank)
{
    ...
    if (currentTank)
    {
        if (Player* nextTank = GetNextKalecgosSurfaceTankInPortalRotation(
                group, state, currentTank->GetGUID());
    ...
    for (ObjectGuid guid : state.tankPortalRotationGuids)
    {
        ...
        if (!tank || !IsKalecgosTankPortalEligibleCandidate(tank))
            continue;
    ...
    PlayerbotAI* botAI, Player* bot, Group* group, KalecgosEncounterState& state)
{
    ...
    if (outgoingTank && currentTank &&
        outgoingTank->GetGUID() == currentTank->GetGUID())
    {
        currentTank = GetNextKalecgosSurfaceTankInPortalRotation(
            group, state, outgoingTank->GetGUID());
    }

    if (!currentTank)
    {
        currentTank = GetFirstKalecgosSurfaceAssignedTank(
...
void AdvanceKalecgosTankPortalRotation(
    KalecgosEncounterState& state, ObjectGuid tankGuid)
{
...
bool CanKalecgosBotEnterRift(
    Group* group, Player* candidate, const KalecgosEncounterState& state)
{
    ...
    Group* group, const KalecgosEncounterState& state)
{
    ...
    {
        const uint8 blastedGroup =
            GetKalecgosAssignedGroup(state, state.blastedPlayerGuid);
    ...
    {
        const uint8 entrantGroup =
            GetKalecgosAssignedGroup(state, state.firstEntrantGuid);
    ...
    Player* member, uint8 groupIndex)
{
    ...
    {
        groupHasTank[groupIndex] = groupHasTank[groupIndex] ||
    ...
    bool preferMissingFlag = false)
{
    ...
    if (bestGroup != KALECGOS_INVALID_GROUP ||
        !requiredFlags || !preferMissingFlag)
    {
        return bestGroup;
    }
...
void EnsureKalecgosGroupAssignments(PlayerbotAI* botAI, Player* bot)
{
    ...
    if (!HasKalecgosTankAssignment(
            state.tankAssignmentGuids, state.activeRiftOutgoingTankGuid))
    {
        state.activeRiftOutgoingTankGuid = ObjectGuid::Empty;
    }
...
void RecordKalecgosSpectralBlastTarget(PlayerbotAI* botAI, Player* bot)
{
    ...
    if (IsKalecgosAssignedTank(state, bot))
    {
        AnnounceKalecgosTankTransition(
            ...
            "Tank %tank has been sent to the Spectral Realm!",
            {{"%tank", bot->GetName()}});
    }
}
...
void RecordKalecgosSpectralRealmEnter(PlayerbotAI* botAI, Player* bot)
{
    ...
    bool const wasBlastedTank = state.blastedPlayerGuid == guid;
    bool const wasCurrentTank = state.currentTankGuid == guid;
    bool const wasOutgoingTank = state.activeRiftOutgoingTankGuid == guid;

    if (wasCurrentTank)
        replacementTank = GetFirstKalecgosSurfaceAssignedTank(group, state, guid);
    ...
    if (IsKalecgosAssignedTank(state, bot))
    {
        if (wasBlastedTank && wasCurrentTank && replacementTank)
        {
            AnnounceKalecgosTankTransition(
                botAI,
                "kalecgos_tank_taking_over",
                "Tank %incoming is moving to the tank position to take aggro on Kalecgos from tank %outgoing!",
                {
                    {"%incoming", replacementTank->GetName()},
                    {"%outgoing", bot->GetName()}
                });
        }
        ...
        if (wasOutgoingTank)
        {
            ...
            if (currentTank && currentTank->GetGUID() != guid)
            {
                AnnounceKalecgosTankTransition(
                    botAI,
                    "kalecgos_tank_entering_spectral_realm",
                    "Tank %outgoing is entering the Spectral Realm after tank %incoming is in the tank position and has aggro on Kalecgos!",
                    {
                        {"%incoming", currentTank->GetName()},
                        {"%outgoing", bot->GetName()}
                    });
            }
        }
        ...
        {
            if (Player* nextTank =
                    GetFirstKalecgosSurfaceAssignedTank(group, state, guid))
            {
