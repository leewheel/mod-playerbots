...
bool VanillaAnubrekhanChooseTargetAction::Execute(Event /*event*/)
{
...
bool VanillaAnubrekhanPositionAction::Execute(Event /*event*/)
{
    ...
    if (inPhase)
    {
        if (botAI->IsMainTank(bot))
        {
            ...
            return MoveInside(VANILLA_NAXX_MAP_ID, 3272.49f, -3476.27f, bot->GetPositionZ(), 3.0f, MovementPriority::MOVEMENT_COMBAT);
    }
    return false;
}
