...
bool VanillaGluthChooseTargetAction::Execute(Event /*event*/)
{
    ...
    else
    {
        for (Unit* t : target_zombies)
        {
            ...
            {
                if (target == nullptr ||
                    target->GetDistance2d(helper.mainTankPos.first, helper.mainTankPos.second) >
                        t->GetDistance2d(helper.mainTankPos.first, helper.mainTankPos.second))
                    target = t;
            }
        }
        if (target == nullptr)
...
bool VanillaGluthPositionAction::Execute(Event /*event*/)
{
    if (!helper.UpdateBossAI())
        return false;

    // Vanilla Naxx40 is 40-man, treat as 25-man logic
    ...
    if (botAI->IsMainTank(bot) || botAI->IsAssistTankOfIndex(bot, 0))
    {
        ...
        {
            // Vanilla Naxx40 uses a single mainTankPos
            if (MoveTo(VANILLA_NAXX_MAP_ID, helper.mainTankPos.first, helper.mainTankPos.second, bot->GetPositionZ(), false, false, false,
                       false, MovementPriority::MOVEMENT_COMBAT))
                return true;

            return MoveInside(VANILLA_NAXX_MAP_ID, helper.mainTankPos.first, helper.mainTankPos.second, bot->GetPositionZ(), 2.0f,
    ...
    else if (botAI->IsRangedDps(bot))
    {
        if (raid25)
        {
            if (botAI->GetClassIndex(bot, CLASS_HUNTER) == 0)
                return MoveInside(VANILLA_NAXX_MAP_ID, helper.leftSlowDownPos.first, helper.leftSlowDownPos.second, bot->GetPositionZ(), 0.0f,
                                  MovementPriority::MOVEMENT_COMBAT);

            if (botAI->GetClassIndex(bot, CLASS_HUNTER) == 1)
                return MoveInside(VANILLA_NAXX_MAP_ID, helper.rightSlowDownPos.first, helper.rightSlowDownPos.second, bot->GetPositionZ(), 0.0f,
                                  MovementPriority::MOVEMENT_COMBAT);
        }
        return MoveInside(VANILLA_NAXX_MAP_ID, helper.rangedPos.first, helper.rangedPos.second, bot->GetPositionZ(), 3.0f,
                          MovementPriority::MOVEMENT_COMBAT);
    }
    else if (botAI->IsHeal(bot))
        return MoveInside(VANILLA_NAXX_MAP_ID, helper.healPos.first, helper.healPos.second, bot->GetPositionZ(), 0.0f,
                          MovementPriority::MOVEMENT_COMBAT);
    return false;
}

bool VanillaGluthSlowdownAction::Execute(Event /*event*/)
{
    if (!helper.UpdateBossAI())
        return false;

    // Vanilla Naxx40 is 40-man, enable slowdown
    bool raid25 = bot->GetRaidDifficulty() == RAID_DIFFICULTY_25MAN_NORMAL ||
                  bot->GetRaidDifficulty() == RAID_DIFFICULTY_10MAN_HEROIC;
    if (!raid25)
        return false;
