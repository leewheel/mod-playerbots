...
bool VanillaRazuviousUseObedienceCrystalAction::Execute(Event /*event*/)
{
    ...
    else
    {
        ...
        // Only 10-man normal uses obedience crystals.
        // 40-man (RAID_DIFFICULTY_10MAN_HEROIC) has NO crystals — priests
        // must use Mind Control on the Death Knight Understudies.
        ...
        else
        {
            ...
            for (auto i = attackers.begin(); i != attackers.end(); ++i)
            {
                ...
                if (!unit)
                    continue;
                if (botAI->EqualLowercaseName(unit->GetName(), "death knight understudy") || unit->GetEntry() == 16803)
                {
                    target = unit;
                    break;
                }
...
bool VanillaRazuviousTargetAction::Execute(Event /*event*/)
{
    if (!helper.UpdateBossAI())
        return false;
