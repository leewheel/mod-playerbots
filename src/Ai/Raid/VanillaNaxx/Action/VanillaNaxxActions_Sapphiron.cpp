...
bool VanillaSapphironGroundPositionAction::Execute(Event /*event*/)
{
    ...
    if (botAI->IsMainTank(bot))
    {
        if (AI_VALUE2(bool, "has aggro", "current target"))
            return MoveTo(VANILLA_NAXX_MAP_ID, helper.mainTankPos.first, helper.mainTankPos.second, helper.GENERIC_HEIGHT, false, false, false,
    ...
    if (helper.JustLanded())
    {
        ...
        if (MoveTo(VANILLA_NAXX_MAP_ID, posX, posY, helper.GENERIC_HEIGHT, false, false, false, false, MovementPriority::MOVEMENT_COMBAT))
            return true;

        return MoveInside(VANILLA_NAXX_MAP_ID, posX, posY, helper.GENERIC_HEIGHT, 2.0f, MovementPriority::MOVEMENT_COMBAT);
    }
    else
    {
        std::vector<float> dest;
        if (helper.FindPosToAvoidChill(dest))
            return MoveTo(VANILLA_NAXX_MAP_ID, dest[0], dest[1], dest[2], false, false, false, false, MovementPriority::MOVEMENT_COMBAT);
    }
    return false;
}

bool VanillaSapphironFlightPositionAction::Execute(Event /*event*/)
{
    if (!helper.UpdateBossAI())
        return false;
...
bool VanillaSapphironFlightPositionAction::MoveToNearestIcebolt()
{
    ...
    for (GroupReference* ref = group->GetFirstMember(); ref; ref = ref->next())
    {
        ...
        if (VanillaNaxxSpellIds::HasAnyAura(member, {VanillaNaxxSpellIds::Icebolt}) ||
