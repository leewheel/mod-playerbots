...
bool VanillaThaddiusAttackNearestPetAction::isUseful()
{
...
bool VanillaThaddiusAttackNearestPetAction::Execute(Event /*event*/)
{
    Unit* target = helper.GetNearestPet();
    ...
    if (botAI->IsTank(bot) && AI_VALUE2(bool, "has aggro", "current target"))
    {
        ...
        return MoveTo(VANILLA_NAXX_MAP_ID, posForTank.first, posForTank.second, helper.tankPosZ, false, false, false, false, MovementPriority::MOVEMENT_COMBAT);
    }
    if (botAI->IsRanged(bot))
    {
        std::pair<float, float> posForRanged = helper.PetPhaseGetPosForRanged();
        return MoveTo(VANILLA_NAXX_MAP_ID, posForRanged.first, posForRanged.second, helper.tankPosZ, false, false, false, false, MovementPriority::MOVEMENT_COMBAT);
    }
    return false;
}

bool VanillaThaddiusMoveToPlatformAction::isUseful() { return true; }

bool VanillaThaddiusMoveToPlatformAction::Execute(Event /*event*/)
{
...
bool VanillaThaddiusMovePolarityAction::isUseful()
{
    return !botAI->IsMainTank(bot) || AI_VALUE2(bool, "has aggro", "current target");
}

bool VanillaThaddiusMovePolarityAction::Execute(Event /*event*/)
{
    std::vector<std::pair<float, float>> position = {
        // left melee
        {3508.29f, -2920.12f},
    ...
    uint32 idx;
    if (VanillaNaxxSpellIds::HasAnyAura(bot,
            {VanillaNaxxSpellIds::NegativeCharge, VanillaNaxxSpellIds::NegativeChargeStack, VanillaNaxxSpellIds::NegativePolarity}) ||
        botAI->HasAura("negative charge", bot, false, false, -1, true))
    {
        idx = 0;
    }
    else if (VanillaNaxxSpellIds::HasAnyAura(bot,
                 {VanillaNaxxSpellIds::PositiveCharge, VanillaNaxxSpellIds::PositiveChargeStack, VanillaNaxxSpellIds::PositivePolarity}) ||
