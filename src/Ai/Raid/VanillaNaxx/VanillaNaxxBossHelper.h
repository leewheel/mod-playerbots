#ifndef PLAYERBOTS_VANILLANAXXBOSSHELPER_H
#define PLAYERBOTS_VANILLANAXXBOSSHELPER_H
...
const uint32 VANILLA_NAXX_MAP_ID = 533;

template <class BossAiType>
class GenericVanillaBossHelper : public AiObject
{
public:
    GenericVanillaBossHelper(PlayerbotAI* botAI, std::string name) : AiObject(botAI), _name(name) {}
...
class VanillaKelthuzadBossHelper : public AiObject
{
public:
    VanillaKelthuzadBossHelper(PlayerbotAI* botAI) : AiObject(botAI) {}
...
class VanillaRazuviousBossHelper : public AiObject
{
public:
    VanillaRazuviousBossHelper(PlayerbotAI* botAI) : AiObject(botAI) {}
    bool UpdateBossAI()
    {
        if (!bot->IsInCombat())
            Reset();
...
class VanillaSapphironBossHelper : public AiObject
{
public:
    ...
    VanillaSapphironBossHelper(PlayerbotAI* botAI) : AiObject(botAI) {}
    bool UpdateBossAI()
    {
        if (!bot->IsInCombat())
            Reset();
    ...
    bool WaitForExplosion()
    {
        ...
        for (GroupReference* ref = group->GetFirstMember(); ref; ref = ref->next())
        {
            ...
            if (member &&
                (VanillaNaxxSpellIds::HasAnyAura(member, {VanillaNaxxSpellIds::Icebolt}) ||
    ...
    {
        Aura* aura = VanillaNaxxSpellIds::GetAnyAura(bot, {VanillaNaxxSpellIds::Chill});
        if (!aura)
        {
...
class VanillaGluthBossHelper : public AiObject
{
public:
    // Vanilla Naxx40 uses a single tank position (40-man, not 10/25)
    const std::pair<float, float> mainTankPos = {3278.29f, -3162.06f};
    ...
    VanillaGluthBossHelper(PlayerbotAI* botAI) : AiObject(botAI) {}
    bool UpdateBossAI()
    {
        if (!bot->IsInCombat())
            Reset();
    ...
    bool BeforeDecimate()
    {
        ...
        if (VanillaNaxxSpellIds::MatchesAnySpellId(info, {VanillaNaxxSpellIds::Decimate}))
            return true;
...
class VanillaFourHorsemenBossHelper : public AiObject
{
public:
    ...
    VanillaFourHorsemenBossHelper(PlayerbotAI* botAI) : AiObject(botAI) {}
    bool UpdateBossAI()
    {
        if (!bot->IsInCombat())
            Reset();
    ...
    // Vanilla Naxx40 is 40-man, use 25-man logic for attracters
    bool IsAttracter(Player* bot)
    {
        Difficulty diff = bot->GetRaidDifficulty();
        // 10MAN_HEROIC = Vanilla Naxx 40-man, treat like 25-man
        if (diff == RAID_DIFFICULTY_25MAN_NORMAL || diff == RAID_DIFFICULTY_10MAN_HEROIC)
        {
...
class VanillaThaddiusBossHelper : public AiObject
{
public:
    ...
    VanillaThaddiusBossHelper(PlayerbotAI* botAI) : AiObject(botAI) {}
    bool UpdateBossAI()
    {
        if (!bot->IsInCombat())
            Reset();
